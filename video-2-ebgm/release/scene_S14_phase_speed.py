...
class S14_PhaseSpeed(Scene):
    ...
    def construct(self):
        ...
        title = label("Rotation, Phase, Speed", UP * 3.05, color=TEXT_PRIMARY, scale=0.55, bold=True)
        ...
        deg0 = label("0°", x0 + UP * 0.38, color=ACCENT_MINT, scale=0.25, bold=True)
        deg11 = label("11°", x11 + UP * 0.38, color=ACCENT_CYAN, scale=0.25, bold=True)
        deg22 = label("22°", x22 + UP * 0.38, color=ACCENT_LAVENDER, scale=0.25, bold=True)

        acc98 = label("98\\%", x0 + LEFT * 0.52 + UP * 0.10, color=ACCENT_MINT, scale=0.30, bold=True)
        acc94 = label("94\\%", x11 + RIGHT * 0.52, color=ACCENT_CYAN, scale=0.30, bold=True)
        acc88 = label("88\\%", x22 + RIGHT * 0.52, color=ACCENT_LAVENDER, scale=0.30, bold=True)

        y_lbl = label("accuracy", axes.get_left() + LEFT * 0.45 + UP * 0.35, color=TEXT_MUTED, scale=0.25)
        x_lbl = label("rotation angle", axes.get_bottom() + DOWN * 0.28, color=TEXT_MUTED, scale=0.25)
        ...
        poses = Group(
            pose_thumbnail(0, axes.c2p(0, 80) + DOWN * 0.85),
            pose_thumbnail(11, axes.c2p(11, 80) + DOWN * 0.85),
            pose_thumbnail(22, axes.c2p(22, 80) + DOWN * 0.85)
        )
        ...
        # B2: is phase necessary?
        question = label("Is phase necessary?", DOWN * 2.45, color=TEXT_PRIMARY, scale=0.45, bold=True)
        beat_to(seg_end(T, 2), FadeIn(question, shift=UP * 0.08), curve.animate.set_stroke(width=5.0))
        ...
        left_title = label("No phase", LEFT * 3.35 + UP * 2.25, color=ACCENT_CORAL, scale=0.43, bold=True)
        right_title = label("With phase", RIGHT * 3.35 + UP * 2.25, color=ACCENT_MINT, scale=0.43, bold=True)
        ...
        err52 = label("5.2 px", LEFT * 3.35 + DOWN * 1.55, color=ACCENT_CORAL, scale=0.55, bold=True)
        rec67 = label("recognition 67\\%", LEFT * 3.35 + DOWN * 2.05, color=ACCENT_CORAL, scale=0.30, bold=True)

        beat_to(seg_end(T, 4), FadeOut(chart_group), Create(split), FadeIn(left_title), FadeIn(right_title), FadeIn(left_face_card), FadeIn(right_face_card), FadeIn(left_refs), LaggedStart(*[Create(l) for l in left_lines], lag_ratio=0.05), FadeIn(left_drifts), FadeIn(err52), FadeIn(rec67))

        err16 = label("1.6 px", RIGHT * 3.35 + DOWN * 1.55, color=ACCENT_MINT, scale=0.55, bold=True)
        rec88 = label("recognition 88\\%", RIGHT * 3.35 + DOWN * 2.05, color=ACCENT_MINT, scale=0.30, bold=True)
        anchor = label("phase = anchor", ORIGIN + DOWN * 2.75, color=ACCENT_MINT, scale=0.40, bold=True)

        beat_to(seg_end(T, 6), FadeIn(right_refs), LaggedStart(*[Create(l) for l in right_lines], lag_ratio=0.05), FadeIn(right_drifts), FadeIn(err16), FadeIn(rec88), FadeIn(anchor), right_title.animate.set_color(ACCENT_MINT))
        ...
        pipeline_title = label("Separate Work", UP * 2.65, color=TEXT_PRIMARY, scale=0.50, bold=True)
        
        pipeline_face = load_face("s8_face", height=1.2)
        pipeline_img = make_face_card(pipeline_face).move_to(LEFT * 5.0 + UP * 0.3)
        img_lbl = label("image", LEFT * 5.0 + DOWN * 0.65, color=TEXT_MUTED, scale=0.28, bold=True)
        
        extract = RoundedRectangle(width=2.75, height=1.28, corner_radius=0.10, color=ACCENT_CYAN, stroke_width=1.7, fill_color=ACCENT_CYAN, fill_opacity=0.08).move_to(LEFT * 1.85 + UP * 0.3)
        extract_lbl = label("Extraction", LEFT * 1.85 + UP * 0.48, color=ACCENT_CYAN, scale=0.33, bold=True)
        extract_sub = label("build the graph", LEFT * 1.85 + UP * 0.12, color=TEXT_MUTED, scale=0.24)
        extract_badge = label("once (x1)", LEFT * 1.85 + DOWN * 0.24, color=ACCENT_CYAN, scale=0.25, bold=True)
        
        match = RoundedRectangle(width=2.85, height=1.28, corner_radius=0.10, color=ACCENT_LAVENDER, stroke_width=1.7, fill_color=ACCENT_LAVENDER, fill_opacity=0.08).move_to(RIGHT * 1.65 + UP * 0.3)
        match_lbl = label("Matching", RIGHT * 1.65 + UP * 0.48, color=ACCENT_LAVENDER, scale=0.33, bold=True)
        match_sub = label("compare scores", RIGHT * 1.65 + UP * 0.12, color=TEXT_MUTED, scale=0.24)
        match_badge = label("many (xN)", RIGHT * 1.65 + DOWN * 0.24, color=ACCENT_LAVENDER, scale=0.25, bold=True)
        ...
        for i in range(6):
            ...
            r_thumb = i // 3
            c_thumb = i % 3
            pos_thumb = RIGHT * 4.6 + np.array([c_thumb * 0.45, 0.52 - r_thumb * 0.52, 0])
            card_thumb.move_to(pos_thumb)
            db.add(card_thumb)
            
        db_lbl = label("database", RIGHT * 5.05 + DOWN * 0.65, color=TEXT_MUTED, scale=0.28, bold=True)
        ...
        speed = label("~1000x faster", DOWN * 1.65, color=ACCENT_MINT, scale=0.62, bold=True)
        fast_enough = label("fast enough for large databases", DOWN * 2.20, color=TEXT_MUTED, scale=0.28)
